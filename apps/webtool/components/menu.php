...
class Menu extends MMenuBar
{
    ...
    public function __construct()
    {
        ...
        foreach ($actions as $i => $group) {
            ...
            $hasAccessGroup = ($group[ACTION_ACCESS] == '') || ($login &&  $login->isMemberOf($group[ACTION_TRANSACTION]));
            if ($hasAccessGroup) {
                ...
                foreach ($groupActions as $j => $action) {
                    ...
                    $hasAccessAction = ($action[ACTION_ACCESS] == '') || ($login && $login->isMemberOf($group[ACTION_TRANSACTION]));
                    if ($hasAccessAction) {
